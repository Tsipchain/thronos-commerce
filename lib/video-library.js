...
function normalizeVideo(record, tenantId) {
  ...
  return {
    ...
    productId: text(record && record.productId),
    categoryId: text(record && record.categoryId),
...
function resolveVideoGuide(videos, { productId, categoryId }) {
  const published = videos.filter((v) => v.published);
  const byProduct = productId ? published.find((v) => v.productId === productId) : null;
  if (byProduct) return { video: byProduct, source: 'product' };
  const byCategory = categoryId ? published.find((v) => v.categoryId === categoryId) : null;
  if (byCategory) return { video: byCategory, source: 'category' };
  const defaultGuide = published.find((v) => v.featured && !v.productId && !v.categoryId);
  if (defaultGuide) return { video: defaultGuide, source: 'default' };
  return null;
}

module.exports = { normalizeVideo, listVideos, slugify, resolveVideoGuide };
